...
public class SearchUtils {
    ...
    public static List<WordBean> getDefaultWordList() {
        List<WordBean> list = new ArrayList<>();
        String[] word = {"abandon", "accordance", "advent", "aerial", "annex", "benevolent", "bouquet", "brag",
        ...
        String[] trans = {"抛弃", "按照", "到来", "空中的", "强占", "仁慈的", "花束",
    ...
    public static List<WordBean> getAllWordLists(Context context) {
        SharedPreferences preferences = context.getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        String json = preferences.getString(KEY_WORD_LIST, "");
        if (TextUtils.isEmpty(json)) {
            return getDefaultWordList(); // Return default list if JSON is empty
        }
        Gson gson = new Gson();
        Type type = new TypeToken<List<WordBean>>() {}.getType();
        return gson.fromJson(json, type);
    }

    public static void saveAllWordLists(Context context, List<WordBean> wordList) {
        SharedPreferences preferences = context.getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        Gson gson = new Gson();
        String json = gson.toJson(wordList);
        editor.putString(KEY_WORD_LIST, json);
        editor.apply();
    }

    public static List<WordBean> addWordToList(Context context, WordBean wordBean) {
        List<WordBean> allWordList = getAllWordLists(context);
        allWordList.add(wordBean);
        saveAllWordLists(context, allWordList);
        return allWordList;
    }

    public static List<WordBean> deleteWordFromList(Context context, WordBean wordBean) {
        List<WordBean> allWordList = getAllWordLists(context);
        allWordList.remove(wordBean);
        saveAllWordLists(context, allWordList);
        return allWordList;
    }
}
